...
.modal-themed .modal-body::-webkit-scrollbar{ width:8px; }
.modal-themed .modal-body::-webkit-scrollbar-track{ background:var(--bg); border-radius:8px; }
.modal-themed .modal-body::-webkit-scrollbar-thumb{ background:var(--red); border-radius:8px; }
.modal-themed .modal-body::-webkit-scrollbar-thumb:hover{ background:var(--red-dark); }
.modal-themed .modal-body{ scrollbar-width:thin; scrollbar-color:var(--red) var(--bg); }
...
$(function () {
    ...
    $("#sortableTable").sortable({
        ...
        update: function () {
            ...
            $.ajax({
                url: "{{ route('inventory-projek.reorder') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    order: order
...
$(document).ready(function () {
    ...
    function renderTable(dataToRender = projekData) {
        ...
        dataToRender.forEach((item, index) => {
            ...
            let row = `
            <tr data-id="${item.id}">
                <td class="text-center"><span class="drag-handle" style="cursor:grab; color:#999;">⋮⋮</span></td>
                <td>${index + 1}</td>
